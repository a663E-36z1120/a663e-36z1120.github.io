...
document.addEventListener("DOMContentLoaded", () => {
  ...
  var wasMobile = false;
  var updateInterval; // milliseconds

  // device detection
  if((navigator.userAgentData && navigator.userAgentData.mobile) || /(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|ipad|iris|kindle|Android|Silk|lge |maemo|midp|mmp|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows (ce|phone)|xda|xiino/i.test(navigator.userAgent) 
  ...
  function checkAndToggleMobile() {
    ...
    const newWasMobile = navigator.userAgentData && navigator.userAgentData.mobile;

    if (wasMobile !== newWasMobile) {
      wasMobile = newWasMobile;
      location.reload();
    }
  ...
  // 3. Survival Rule: A cell survives only if has 2 or 3 neighbors alive.
    ...
    if (isMobile) {
      ...
      document.querySelector(".side-box").style.display = "none"
    } else {
